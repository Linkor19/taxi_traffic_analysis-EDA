...
-- SELECT vendor_id, COUNT(*), ROUND(COUNT(*)/(SELECT COUNT(*) FROM 'taxi_data/data/train.csv')*100,2) -- percentage of suppliers
-- FROM 'taxi_data/data/train.csv'
-- GROUP BY vendor_id;

-- ------------------------------------------------------------------------------------------

-- SELECT store_and_fwd_flag, COUNT(*), ROUND(COUNT(*)/(SELECT COUNT(*) FROM 'taxi_data/data/train.csv')*100,2)    --data persistence
-- FROM 'taxi_data/data/train.csv'
...
-- 	(SELECT COUNT(*) FROM 'taxi_data/test_df.csv' C WHERE C.NATName_p = A.NATName_p) * 100,2) AS prc,
-- 	SUM(cnt) OVER (PARTITION BY A.NATName_p) AS gen_cnt
-- FROM 'taxi_data/test_df.csv' A INNER JOIN period B
-- ON A.id = B.id
...
-- FROM rout A INNER JOIN 'taxi_data/test_df.csv' B
...
-- FROM period A INNER JOIN 'taxi_data/data/train.csv' B
-- ON A.id = B.id
...
-- FROM period A INNER JOIN 'taxi_data/data/train.csv' B   -- for python export
-- ON A.id = B.id
-- GROUP BY date
-- ORDER BY date;

-- SELECT date, COUNT(B.id) AS ven_1, COUNT(C.id) AS ven_2
-- FROM period A LEFT JOIN 'taxi_data/data/train.csv' B
-- ON A.id = B.id AND B.vendor_id = 1
-- LEFT JOIN 'taxi_data/data/train.csv' C
-- ON A.id = C.id AND C.vendor_id = 2
-- GROUP BY date
-- ORDER BY date;

-- SELECT date, COUNT(*)
-- FROM 'taxi_data/data/train.csv' A INNER JOIN period B
-- ON A.id = B.id
-- GROUP BY date
...
WITH period AS (
	SELECT date, STRFTIME('%m', date) AS month,
		CASE 
				...
				ELSE 'autumn'
				END season
	FROM 'taxi_data/data/weather.csv'
)
...
FROM period A INNER JOIN 'taxi_data/data/weather.csv' B
ON A.date = B.date
...
-- FROM period A INNER JOIN 'taxi_data/test2_df.csv' B
-- ON A.date = B.date
-- GROUP BY ROLLUP (season, month)
-- ORDER BY month;
